...
@Path("cron/cache/latest/urltweet")
public class UrlTweetLatestCacheQueueResource {

    private static final Logger logger = Logger.getLogger(UrlTweetLatestCacheQueueResource.class.getName());

    @GET
    public Response fetchLinkTweets(){
     
           Queue queue = QueueFactory.getQueue("updateLatestTweetCache");

           queue.add(url("/linktweets.json?category_name=b&rpp=6&days_ago=0&my_host=us").method(Method.GET));
           queue.add(url("/linktweets.json?category_name=b&rpp=6&days_ago=0&my_host=us").method(Method.GET));
           queue.add(url("/linktweets.json?category_name=b&rpp=6&days_ago=0&my_host=us").method(Method.GET));
